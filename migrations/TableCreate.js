...
con.connect(function (err) {
    ...
    let C_cards = "CREATE TABLE IF NOT EXISTS cards (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(20), type VARCHAR(20), dmg INT, role VARCHAR(20), effect TINYINT(1) NOT NULL DEFAULT 0, animation VARCHAR(40), img VARCHAR(40), description VARCHAR(80), music VARCHAR(40));";
    let C_cards_effect = "CREATE TABLE IF NOT EXISTS cards_effect (id INT AUTO_INCREMENT PRIMARY KEY, id_card INT, id_e INT);";
    let C_effect = "CREATE TABLE IF NOT EXISTS effect (id INT AUTO_INCREMENT PRIMARY KEY, description VARCHAR(80), dgt INT, state VARCHAR(80), target VARCHAR(10), effect_trigger VARCHAR(20), death TINYINT(1) NOT NULL DEFAULT 0, steal TINYINT(1) NOT NULL DEFAULT 0, can_generate TINYINT(1) NOT NULL DEFAULT 0, create_id INT, card_effect INT);";
